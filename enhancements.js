  ...
  ready(async()=>{
    ...
    if(window.gsap&&window.ScrollTrigger){
      ...
      extraSelectors.forEach(sel=>document.querySelectorAll(sel).forEach((el,i)=>{
        ...
        animated.add(el);el.classList.add('zeph-enter');
        gsap.from(el,{autoAlpha:0,y:34,filter:'blur(7px)',scale:.985,duration:.9,delay:Math.min(i*.035,.14),ease:'power3.out',scrollTrigger:{trigger:el,start:'top 91%',once:true}});
      }));

      document.querySelectorAll('section').forEach(section=>{
        const targets=[...section.querySelectorAll(':scope > .wrap > h1,:scope > .wrap > h2,:scope > .wrap > p,:scope > .wrap > .eyebrow')];
        targets.forEach((el,i)=>{if(animated.has(el)||el.classList.contains('reveal'))return;animated.add(el);gsap.from(el,{autoAlpha:0,y:28,filter:'blur(6px)',duration:.82,delay:i*.04,ease:'power3.out',scrollTrigger:{trigger:el,start:'top 92%',once:true}})});
      });
